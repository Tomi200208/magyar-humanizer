...
1. Töltsd fel a `magyar-humanizer.skill` ZIP-fájlt a Claude beállításai között a "Skills" szekcióban.
2. Ha kézzel akarod, csomagold ki, és töltsd fel a `SKILL.md`-t a referencia-fájlokkal együtt.

### Claude Code

```bash
mkdir -p ~/.claude/skills/magyar-humanizer
unzip magyar-humanizer.skill -d ~/.claude/skills/
```

Vagy git-tel közvetlenül:

```bash
git clone https://github.com/Tomi200208/magyar-humanizer ~/.claude/skills/magyar-humanizer
```

### OpenCode

```bash
mkdir -p ~/.config/opencode/skills/magyar-humanizer
unzip magyar-humanizer.skill -d ~/.config/opencode/skills/
```
...
2. **Audit** — *"Mitől hangzik még mindig AI-szerűnek?"* — saját kritika
3. **Végső átírás** (Final) — az audit alapján finomítva
...
2. Indokold röviden, miért gondolod AI-jelnek (nem stilisztikai preferencia).
3. Adj 2-3 példát ugyanarra a mintára különböző kontextusból.
